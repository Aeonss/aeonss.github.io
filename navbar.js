class SiteNavbar extends HTMLElement {
  connectedCallback() {
    ...
    const navLinkBase =
      "font-mono text-[10.5px] tracking-[0.1em] uppercase no-underline px-2 sm:px-3.5 py-1.5 rounded-[3px] transition-colors duration-[180ms]";
    const navLinkIdle =
    ...
    this.innerHTML = `
      <nav class="site-nav fixed top-0 left-0 right-0 z-[100] flex flex-wrap items-center gap-y-1 px-4 sm:px-8 min-h-14 mt-6">
        <a href="index.html" class="nav-brand font-display text-xl font-bold tracking-[0.08em] no-underline mr-4 sm:mr-9 flex-shrink-0">
          <img
            src="icon.gif"
            class="h-[45px] sm:h-[70px]"
          />
        </a>

        <div class="flex gap-0.5 flex-1 items-center flex-wrap">
